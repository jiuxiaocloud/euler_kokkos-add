  ...
  namespace muscl
  {
    ...
    public:
      InitImplodeFunctor2D(HydroParams params,
                           ...
                           DataArray2d Udata) : HydroBaseFunctor2D(params), iparams(iparams), Udata(Udata){};
    ...
    { // NOTE：BlastInit Kokkos here

    public:
      InitBlastFunctor2D(HydroParams params,
                         BlastParams bParams,
                         DataArray2d Udata) : HydroBaseFunctor2D(params), bParams(bParams), Udata(Udata){};
      ...
      void operator()(const int &index) const
      {
        ...
        const real_t radius2 = bParams.if_cop ? blast_center_x : (blast_radius * blast_radius);
#if COP
        const real_t cop_center_x = bParams.cop_center_x;
        const real_t cop_center_y = bParams.cop_center_y;
        const real_t cop_radius = bParams.cop_radius;
        const real_t cop_radius2 = bParams.if_cop ? (cop_radius * cop_radius) : cop_center_x;
        const real_t cop_y1_in = bParams.cop_y1_in;
        const real_t cop_y1_out = bParams.cop_y1_out;
        const real_t cop_density_in = bParams.cop_density_in;
        const real_t cop_pressure_in = bParams.cop_pressure_in;
#endif
        int i, j;
        index2coord(index, i, j, isize, jsize);
        ...
        real_t d2 = bParams.if_cop ? x : ((x - blast_center_x) * (x - blast_center_x) + (y - blast_center_y) * (y - blast_center_y)); //可以选择激波形状，圆形或shock-wave
#if COP
        real_t dy2 = bParams.if_cop ? ((x - cop_center_x) * (x - cop_center_x) + (y - cop_center_y) * (y - cop_center_y))
                                    : x; //可以选择组分不同区域，圆形或类shock-wave
#endif
        ...
        if (d2 < radius2)
        {
          ...
          Udata(i, j, IV) = 0.0;
#if COP
          Udata(i, j, IDY1) = cop_y1_out * Udata(i, j, ID);
#endif
        }
        else
        {
          ...
          Udata(i, j, IV) = 0.0;
#if COP
          int n = cop_radius / dx + 1;
          if (dy2 < (n - 1) * (n - 1) * dx * dx) //|| dy2 == (n - 1) * (n - 1) * dx * dx)
          {
            Udata(i, j, ID) = cop_density_in;                   //气泡内单独赋值密度以和气泡外区分
            Udata(i, j, IP) = cop_pressure_in / (gamma0 - 1.0); //气泡内单独赋值压力以和气泡外区分
            Udata(i, j, IDY1) = cop_y1_in * Udata(i, j, ID);    //组分气泡必须在激波下游
          }
          else if (dy2 > (n + 1) * (n + 1) * dx * dx)
          {
            Udata(i, j, ID) = blast_density_out;                   //气泡内单独赋值密度以和气泡外区分
            Udata(i, j, IP) = blast_pressure_out / (gamma0 - 1.0); //气泡内单独赋值压力以和气泡外区分
            Udata(i, j, IDY1) = cop_y1_out * Udata(i, j, ID);
          }
          else
          {
            Udata(i, j, ID) = 0.5 * (cop_density_in + blast_density_out);                    // blast_density_out;                                  //
            Udata(i, j, IP) = 0.5 * (cop_pressure_in + blast_pressure_out) / (gamma0 - 1.0); // blast_pressure_out / (gamma0 - 1.0);                //气泡内单独赋值压力以和气泡外区分
            Udata(i, j, IDY1) = 0.5 * (cop_y1_out + cop_y1_in) * Udata(i, j, ID);            //组分取一半，密度取两种的均值
          }
#else
    ...
    public:
      InitFourQuadrantFunctor2D(HydroParams params,
                                ...
                                HydroState U3,
                                real_t xt,
                                real_t yt) : HydroBaseFunctor2D(params), Udata(Udata),
