...
int main(int argc, char *argv[])
{
  ...
  using namespace euler_kokkos;
  {
...
#if COP
    std::string gasname[2] = {"O2", "H2"}; //第2个气体为组分气体
    DataArrayScalar species("species", 2, 8);
    DataArray2d Hia("HiaArgu", 2, 7, 3);
    DataArray2d Hib("HibArgu", 2, 2, 3);
    DataArrayScalarHost speciesHost = Kokkos::create_mirror_view(species);
    DataArray2dHost HiaArguHost = Kokkos::create_mirror_view(Hia);
    DataArray2dHost HibArguHost = Kokkos::create_mirror_view(Hib);
    readSpecie(gasname, speciesHost);
    readHiArgus(gasname, HiaArguHost, HibArguHost, speciesHost); //引用可以作为左值赋值，只是用了const修饰就不行了
    Kokkos::deep_copy(species, speciesHost);
    Kokkos::deep_copy(Hia, HiaArguHost);
    Kokkos::deep_copy(Hib, HibArguHost);

    HydroParams params = HydroParams(species, Hia, Hib); //(species, Omega_table, T_star, delta_star, Hia, Hib); //, Djk, phi, X, Dim, miu, y, species0, species1, aa, T);
    ...
    while (!solver->finished())
    {
      ...
      // std::cout << "prefrom a step" << std::endl;
    } // end solver loop
