...
namespace euler_kokkos
{
  ...
  KOKKOS_INLINE_FUNCTION void cmpflx(const HydroState &qgdnv,
                                     HydroState &flux,
   ...
   */
  template <class HydroState>
  KOKKOS_INLINE_FUNCTION void riemann_approx(const HydroState &qleft,
                                             ...
                                             const HydroParams &params)
  {
    ...
    if (frac != frac) /* Not a Number */
      ...
      frac = frac >= 1.0 ? 1.0 : frac <= 0.0 ? 0.0
                                             : frac;
   ...
   */
  template <class HydroState>
  KOKKOS_INLINE_FUNCTION void riemann_llf(const HydroState &qleft,
                                          const HydroState &qright,
                                          HydroState &qgdnv,
                                          HydroState &flux,
                                          const HydroParams &params)
   ...
   */
  template <class HydroState>
  KOKKOS_INLINE_FUNCTION void riemann_hll(const HydroState &qleft,
                                          const HydroState &qright,
                                          HydroState &qgdnv,
                                          HydroState &flux,
                                          const HydroParams &params)
  {
    ...
    for (int nVar = 0; nVar < HYDRO_2D_NBVAR; nVar++)
    {
      flux[nVar] = (SR * fleft[nVar] - SL * fright[nVar] +
                    SR * SL * (uright[nVar] - uleft[nVar])) /
                   (SR - SL);
    }
    if (std::is_same<HydroState, HydroState3d>::value)
    {
      flux[IW] = (SR * fleft[IW] - SL * fright[IW] +
                  SR * SL * (uright[IW] - uleft[IW])) /
                 (SR - SL);
    }
   ...
   */
  template <class HydroState>
  KOKKOS_INLINE_FUNCTION void riemann_hllc(const HydroState &qleft,
                                           const HydroState &qright,
                                           HydroState &qgdnv,
                                           HydroState &flux,
                                           const HydroParams &params)
  {
    ...
    real_t T = 298.15;
    real_t gamma0 = 0.0;
    get_CopGamma0(T, params, qleft, qright, &gamma0);
    // printf("gamma0=%lf", gamma0);
#else
    real_t gamma0 = params.settings.gamma0;
#endif
    real_t smallr = params.settings.smallr;
    ...
    if (std::is_same<HydroState, HydroState3d>::value)
      ecinl += HALF_F * rl * qleft[IW] * qleft[IW]; // Hl

    real_t etotl = pl * entho + ecinl; // El
    real_t ptotl = pl;
...
// NOTE:添加组分的通量计算项，理论来源是HLLC-add_Flux_yi-s00193-019-00912-4.pdf Page11 Chapter 6
#if COP
    real_t z = (gamma0 - ONE_F) / 2.0 / gamma0;
    real_t z1 = 1.0 / z;
    real_t D = sqrt(rl * rr);
    real_t al = sqrt(gamma0 * pl / D);
    real_t ar = sqrt(gamma0 * pr / D);
    real_t pstarbar = std::pow((al + ar - 0.5 * (gamma0 - 1) * (ur - ul)) / (al / std::pow(pl, z) + ar / std::pow(pr, z)), z1);
    real_t ql = pstarbar > pl ? pow((1 + 0.5 * (gamma0 + 1) / gamma0 * (pstarbar / pl - 1.0)), 0.5) : 1.0;
    real_t qr = pstarbar > pr ? pow((1 + 0.5 * (gamma0 + 1) / gamma0 * (pstarbar / pr - 1.0)), 0.5) : 1.0;
    real_t Sl = ul - al * ql;
    real_t Sr = ur + ar * qr;
    real_t Sstar = (pr - pl + rl * ul * (Sl - ul) - rr * ur * (Sr - ur)) / (rl / (Sl - ul) - rr / (Sr - ur));
    real_t Qstarlag = rl * (Sl - ul) / (Sl - Sstar);
    real_t Qstarrag = rr * (Sr - ur) / (Sr - Sstar); // eq.78的向量外系数
    real_t yl = qleft[IDY1];
    real_t yr = qright[IDY1];
    real_t fluxl = rl * ul * yl;
    real_t fluxr = rr * ur * yr;
    real_t fluxstarl = fluxl + Sl * (Qstarlag * yl - rl * yl);
    real_t fluxstarr = fluxr + Sr * (Qstarrag * yr - rr * yr);
    if (Sl > ZERO_F)
    {
      flux[IDY1] = fluxl;
    }
    else if (Sstar > ZERO_F)
    {
      flux[IDY1] = fluxr;
    }
    else if (Sstar < ZERO_F)
  ...
  // void riemann_roe(const HydroState &qleft,
  //                  const HydroState &qright,
  //                  const HydroState &qLoc,
  //                  HydroState &qgdnv,
  //                  HydroState &flux,
  //                  const HydroParams &params)
  // {
  ...
  //   HydroEigen2d eigen_l, eigen_r; // L Matrix and R Matrix
  //   HydroState2d eigen_value;
  //   get_RoeAverage(qleft, qright, qLoc, eigen_l, eigen_r, eigen_value);
                     ...
                     const HydroParams &params)
  {
    ...
    // else if (params.riemannSolverType == RIEMANN_ROE)
    // {
    //   // NOTE：计算多组分使用的Roe求解器，使用现成的
    //   riemann_roe<HydroState2d>(qleft, qright, qgdnv, flux, params);
    // }
  } // riemann_hydro

  // NOTE:加了组分以后和原来3D的Emax个数是一样的
#if !COP
  /**
   * Wrapper function calling the actual riemann solver.
   */
