...
namespace euler_kokkos
{
  ...
  public:
    ...
    CopyBorderBuf_To_DataArray(DataArray U,
                               DataArray b,
                               int ghostWidth) : U(U), b(b), ghostWidth(ghostWidth){};
  ...
  public:
    ...
    CopyDataArray_To_BorderBuf(DataArray b,
                               DataArray U,
                               int ghostWidth) : b(b), U(U), ghostWidth(ghostWidth){};
    ...
    KOKKOS_INLINE_FUNCTION void operator()(const typename std::enable_if<dimType_ == TWO_D, int>::type &index) const
    {
      ...
      const int isize = U.extent(0); // DataArray2d U,是定义在Device RAM内的View,DataArray2d U("U",isize,jsize,nbvar),可以分别对应extent内的三个值
      const int jsize = U.extent(1);
      const int nbvar = U.extent(2);
      int i, j;
